...
- 데이터의 새 부분을 요청하는 경우 진행중인 요소 처리를 중단하거나 차단하지 않아도 된다.
...
- 일부 변환 로직을 구현해 스트리밍 파이프라인 역할을 하며, 비즈니스 로직 흐름을 더 쉽게 이해할 수 있다
...
- 위와 유사한 검증이나, 인증을 통과하기 위해 Subscriber와 WhiteboxSubscriberProbe의 추가적인 상호작용을 구현해야 함
  ...
  - 한편, 모든 프로세서나 코어를 효율적으로 사용하려면 병렬 처리가 필요하다
    ...
    - 리액티브 스트림 내의 병렬화? Subscriber#onNext 메서드를 병렬로 호출하는 것 -> 하지만 on*** 메서드의 호출은 '스레드 안정성을 보장하는 방식으로 신호를 보내야 하며, 다중 스레드에서 수행되는 경우 외부적인 동기화를 사용해야 한다'고 함.
