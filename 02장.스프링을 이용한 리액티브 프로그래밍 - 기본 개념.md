...
- REST 컨트롤러는 적은 로직 유지, 리액티브 구현 -> TemperatureSensor, RxSseEmitter는 스트림을 SSE 메시지로 변환
- 스프링의 EventBus를 사용하지 않으므로 이식성이 더 높고 스프링 컨텍스트 없이도 테스트가 가능함
- 아무도 수신하지 않을 때는 온도 센서를 탐색하는 오버헤드가 없음 => 리액티프 프로그래밍이 가지는 **능동적 구독**. lazy loading이라 봐도되나? 싶긴 함
