...
- 리액티브 프레임워크 중에서 가장 유명한 라이브러리인 리액터 프로젝트에 대해 알아보자!
  - 리액터 프로젝트의 역사와 배경
  - 리액터 프로젝트의 용어와 API
  - 리액터 프로젝트의 고급 기능
...
- 리액티브 스트림 스펙은 API 및 규칙만 정의하고 **일상적인 사용을 위한 라이브러리**는 제공하지 않음.
- 이에 우리는 리액티브 스트림 스펙 중 가장 인기 있는 구현체 중 하나인 **리액터 프로젝트**에 대해 학습해보자.

#### 리액터 프로젝트 버전 1.x

- 스프링 프레임워크 팀의 개발자들은 처리량이 많은 데이터 처리 프레임워크가 필요하였음

- 이에 리액터 프로젝트 1.0이 만들어짐.

  - 처음부터 **논블로킹 처리**를 지원하도록 설계하였으며, **리액터 패턴, 함수형 프로그래밍 및 리액티브 프로그래밍**과 같은 메시지 처리에 대한 모범 사례를 통합하여 만들었음.

  - **리액터 패턴**
  
    - https://riverandeye.tistory.com/entry/1-Reactor-Pattern
  
    - application이 능동적으로 계속해서 처리하기 위한 루프를 도는 것이 아니라, **이벤트에 반응하는 객체(reactor)**를 만들고, 이벤트가 발생하면 application 대신 reactor가 반응하여 처리하는 것. 이에 따로 **application에서 이벤트를 대기하고 분할하는 작업을 하지 않는 것**이 장점이다.
  
    - 모든 이벤트가 큐에 추가 되고 이벤트는 나중에 별도의 스레드에 의해 처리, 이벤트는 이벤트 핸들러로 발송되어 동기적으로 처리됨
  
      <img src="https://blog.kakaocdn.net/dn/cn4EEy/btqLf59xIei/vgdQXgCKsTye3etZtwKAo0/img.png" alt="img" style="zoom:50%;" />
  
  
  ~~~java
  Environment env = new Environment();
  //builder pattern. Reactor 클래스의 인스턴스 생성
  Reactor reactor = Reactors.reactor()
    												.env(env)
    												.dispatcher(Environment.RING_BUFFER)
    												.get();
  //event consumer 선언 및 , event handler 등록
  ...
  - 내부적으로 이벤트는 Dispatcher에 의해 처리된 후 목적지로 보내진다.
  - 이제 Dispatcher 구현에 따라 이벤트가 동기 또는 비동기적으로 처리될 수 있음
  
  ~~~java
  ... // Environment, Reactor 인스턴스 생성
  // stream 생성. 이 stream은 주어진 리액터와 설정한 채널 기반으로 동작
  ...
  // Streams.defer() : Reactor 클래스의 인스턴스를 추가로 생성
  ...
  - 1번 예시의 경우 채널을 구독하고 들어오는 모든 이벤트를 단계적으로 처리
  - 2번 예시는 리액티브 프로그래밍 기법을 사용해 선언적 처리 흐름을 만듬
  - 리액터 1.x는 그 당시 이벤트를 빠른 속도로 처리하기에 충분했음
  - 리액터 1.x의 단점
    - Back pressure 기능이 없음
    - 오류 처리의 복잡함
    - 이에 리액티브 라이브러리 다음 버전이 생성되었음
  
  #### 리액터 프로젝트 버전 2.x
  
  - 스테판 말디니와 존 브리스빈은 2015년 초에 리액터 2.x 발표 
  - **이벤트 버스 및 스트림 기능을 별도의 모듈로 추출**
  - 리액티브 스트림 스펙을 완벽하게 준수하게 설계함
  - Back pressure 관리, 스레드 처리, 복원력 지원 등 다양한 기능 추가
  
  ~~~java
  stream
    	.retry() // 오류 발생 시 업스트림 작업 다시 실행
  ...
  - 리액터 3의 경우 자바 8을 기준으로 섵개하였음.

### 리액터 프로젝트 필수 요소

- 처음부터 리액터 라이브러리는 비동기 파이프라인을 구축할 때, **콜백 지옥과 깊게 중첩된 코드를 생략하는 목적**으로 설계
- "리액티브 응용 프로그램을 통해 처리된 데이터는 마치 조립 라인을 통해 이동하는 것과 유사하다. 리액터는 컨베이어 벨트와 워크스테이션 역할이라고 할 수 있다"
  - 라이브러리의 목표는 코드 **가독성**을 높이고 **조합성(composability)**을 추가하는 것
  - 순수한 리액티브 스트림 스펙에 커다란 부가 가치를 제공하는 **다양한 종류의 연산자**(위에서의 워크스테이션)를 제공
  - 위 연산자들을 통해 재사용 가능한 실행 그래프를 작성할 수 있으며, 이 실행 그래프는 실행 흐름만 정의하지만, 구독자가 **구독을 했을 때만 데이터 플로가 기동**(lazy loading...! 런타임! 선언적)
  - 비동기 요청의 **결과를 효율적으로 처리**할 수 있게 설계하였으며, 오류 처리 시 **복원력 있는** 코드를 작성할 수 있게 해줌.
  - 라이브러리는 Back Pressure 전파의 일반적인 모드를 모두 지원
  ...
  - 리액터 프로젝트는 동시성에 좌우되지 않도록 설계되었으므로 동시성 모델을 적용하지 않고, 동시에 거의 모든 방식으로 **실행 스레드를 관리할 수 있는 유용한 스케줄러 세트**를 제공 -> 알아서 잘짜도록..
- 위와 같이 여러 장점들이 있으며, API들을 살펴보자.

#### 프로젝트에 리액터 추가하기

- 리액터 프로젝트는 유일하게 리액티브 스트림에 의존성을 가짐을 기억하자.
- 의존성 추가
...
- 0, 1 또는 여러 요소를 생성할 수 있는 **일반적인 리액티브 스트림**을 정의
...
- 또한 구독자는 언제든지 구독을 취소할 수 있고 효과적으로 무한 스트림을 유한 스트림으로 변환할 수 있다
...
- **최대 하나의 요소**를 생성할 수 있는 스트림

- 버퍼 중복과 값비싼 동기화 작업을 생략하기 때문에 효율적으로 사용 가능? -> 미래에 하나만 오면 된는 것

- **응용 프로그램 API가 최대 하나의 원소를 반환하는 경우 유용함**

- 결과적으로 봤을때는 CompletableFuture<T>와 비슷한 용도로 사용할 수는 있음

  - CompletableFuture ➡ 반드시 반환값 반환, 즉시 처리 시작

  - Mono ➡ 구독자가 나타나야만 작업 수행, 리액티브 연산자를 많이 제공, 더 큰 규모의 리액티브 워크플로와 완벽하게 통합 가능
...
- RxJava 2 버전을 위한 타입들이 있지만, 우리가 사용하지 않을 것이고 레거시도 없기에 생략..
  ...
  - RxJava2가 리액티브 스트림과 호환되는 타입은 Flowable 타입만이다.
  - 리액티브 스트림과 호환되는 다른 코드와 통합하려면 RxJava 유형을 Flowable 유형으로 변환해야 한다


#### Flux와 Mono 시퀀스 만들기

- Flux 및 Mono는 데이터를 기반으로 **리액티브 스트림을 생성하는 많은 팩토리 메서드**를 제공
  - Factory Method : 객체 생성을 Factory 클래스로 캡슐화 처리하여 대신 생성하게 하는 생성 디자인 패턴 ➡ 객체 생성에 필요한 과정을 템플릿처럼 미리 구성해두고 생성 과정을 다양하게 처리하여 객체를 유연하게 정할 수 있는 특징이 있다.
...
//fromArray, fromIterable 메서드를 사용해 Flux 스트림으로 변환
Flux<Integer> stream2 = Flux.fromArray(new Integer[]{1, 2, 3}); // 객체 및 컬렉션에서 참조 가능
Flux<Integer> stream3 = Flux.fromIterable(Arrays.asList(9,8,7));
Flux<Integer> stream4 = Flux.range(2010, 9); // 2010, 2011, 2012, ..., 2018

// Mono는 하나의 요소를 대상으로 하기에, nullable, Optional 타입과 함께 자주 사용됨.
// 리액터 프로젝트를 사용하면 just 메서드를 이용해서 요소를 열거하는 방식으로 Flux 및 Mono 시퀀스 생성
Mono<String> stream5 = Mono.just("One");
// justOrEmpty를 사용해 Optional을 Mono로 쉽게 래핑
// 이 외에도 fromSupplier, fromFuture, fromRunnable 메서드 => Supplier를 Mono로 래핑, Future로 매핑, Runnable로 매핑
Mono<String> stream6 = Mono.justOrEmpty(null);
Mono<String> stream7 = Mono.justOrEmpty(Optional.empty());

// HTTP 요청이나 DB query와 같은 비동기 작업을 래핑하는데 매우 유용
Mono<String> stream8 = Mono.fromCallable(() -> httpRequest());
Mono<String> stream8 = Mono.fromCallable(this::httpRequest);

// Flux와 Mono는 from(Publisher<T> p) 팩토리 메서드를 사용해 다른 Publisher 인스턴스를 변환할 수 있음?

Flux<String> empty = Flux.empty(); // 빈 스트림과 오류만 포함하는 스트림
Flux<String> never = Flux.never(); // 완료 메시지, 데이터, 오류에 대해서도 신호를 보내지 않는 스트림
Mono<String> error = Mono.error(new RuntimeException("Unknown id")); // 각 구독자의 onError() 메서드를 통해 항상 오류를 전파하는 시퀀스 생성

// defer : 구독하는 순간에 행동을 결정하는 시퀀스(서로 다른 구독자에 대해 다른 데이터를 생성할 수 있음)
// 1번 예제 -> 실제 구독이 발생해야 유효성 검사를 함
...
- 리액티브 스트림에 의해 생성된 요소를 소비하는 방법을 배워보자.
- Flux와 Mono는 구독 루틴을 훨씬 단순화하는 subscribe() 메서드를 람다 기반으로 재정의한다
  - 아래와 같이 오버로딩한 subscribe의 모든 메서드는 **Disposable 인터페이스**의 인스턴스를 반환
...
// 1번 예제의 경우 제한 없이 요청함
// 2번 예제의 경우 subscription 설정으로 구독을 제어하였다
// 즉, 프로듀서가 제한된 수요를 처리하는 데 더 적합하다면 Subscription 객체로 수요를 제어하거나 요청 제한 연산자를 적용하는 것이 좋다
...
//onNext: 2
//onNext: 3
//onNext: 4 (구독을 취소한 것이기에, onComplete 신호를 수신하지 않음)
~~~

- 또한, Disposable 인스턴스를 취소용으로 사용할 수 있다
...
- 더 필요한 요구 사항이 있다면 리액티브 스트림 스펙에 따라 **Subscriber 인터페이스를 직접 구현**할 수 있음

  ~~~java
  Subscriber<String> subscriber = new Subscriber<String>() {
    //main memory에 저장하여 올바르게 참조할 수 있게 함
...
- 하지만, 위와 같이 구독을 직접 정의하면 **스스로 BackPressure 관리**해야하고 가입자에 대한 모든 **TCK 요구사항을 올바르게 구현**해야 함

- 이에, 리액터 프로젝트에서 제공하는 **BaseSubscriber 클래스**를 상속하는게 훨씬 더 좋은 방법임
...
- 이러한 변화 도구들은 일반적으로 아래와 같이 분류 가능
  ...
  - 시간을 다루는 작업
  - 데이터를 동기적으로 반환
- 대부분 연산자는 기본 동작을 보강하기 위해 다양한 매개변수를 이용해 호출할 수 있음

#### 리액티브 시퀀스의 원소 매핑하기

- Flux 및 Mono는 자바 스트림 API의 map 연산자와 비슷한 동작을 하는 map 연산자를 제공
...
//index: 0, ts: 2018-09-24T03:00:52.0412, value: 2018 
//index: 1, ts: 2018-09-24T03:00:52.0612, value: 2019 
//index: 2, ts: 2018-09-24T03:00:52.0612, value: 2020 
//index: 3, ts: 2018-09-24T03:00:52.0612. value: 2021 
...
- 모든 원소를 수집하고 결과를 Flux.collectList() 및 Flux.collectSortedList()를 사용해 Mono 타입 스트림으로 처리할 수 있음
...
- List 외에도 Map<K, T>, Map<K, Collection<T>>, java.util.stream.Collector를 상속한 모든 형태의 데이터 구조로 변환할 수 있다. 
- distinct 메서드의 경우 모든 원소를 추적하기 때문에 신중하게 사용해야 함!
- Flux.distinctUntilChanged() 연산자는 방금 들어온 값만 확인해서 중복이면 제거해준다. 11122233211 -> 12321
...
Flux.range(1, 500)
  	.index()
  	.scan( 
  		new int[bucketSize], //최근 5개의 원소를 컨테이너로 수집 
      (acc, elem) -> {
        acc[(int)(elem.getT1() % bucketSize)] = elem.getT2(); // 컨테이너의 위치 계산
        return acc;
      }
		)
  	.skip(bucketSize) // 일부 원소를 건너뜀
  	.map(array -> Arrays.stream(array).sum() * 1.0 / bucketSize) // 평균 속도 계산
...
- 리액터 프로젝트는 버퍼링, 윈도우 처리로 스트림에 대한 일괄 처리를 지원
...
-  buffer 연산자는 여러 개의 이벤트를 묶어서 이벤트 컬렉션을 만들어냄.
- 하나의 원소만 가진 많은 작은 요청 대신에 컬렉션을 이용해 요청 횟수를 줄이는 것이 바람직한 경우 편리하게 사용
- 물론 데이터 정합성에 대한 요구 사항을 위배하지 않는 경우에만 해당함
  - 데이터 정합성? 어떤 데이터들이 값이 서로 일치하는 상태

~~~java
// 윈도우윙 예제. 소수일 때마다 숫자 시퀀스
...
<img src="/Users/seongjayong/Learning/book-spring-reactive/image/그림 4.7 flatMap 연산자.png" alt="그림 4.7 flatMap 연산자" style="zoom:50%;" />

- 원 하나마다 사각형과 삼각형을 생성하는 예제. 작업의 결과는 하나의 하위 스트림으로 병합됨!

- flatMap 연산자는 논리적으로 map 과 flatten 의 2가지 작업으로 구성됨

  - map 파트 : 들어오는 각 원소를 리액티브 스트림(T -> Flux<R>)으로 변환
  - flatten 파트 : 생성된 모든 리액티브 시퀀스를 R 타입의 원소를 통과시키는 새로운 리액티브 시퀀스로 병합 (병렬)
...
- 사용자가 좋아하는 책을 확인하는 간단한 알고리즘을 예를 들어보자.
  ...
  public Flux<String>requestBooks(String user) { 
    return Flux.range(1, random.nextInt(3) + 1) //임의의 정수 값 생성
  							.map(i -> "book-" + i) // 각 번호를 책 제목에 매핑
      					.delayElements(Duration.ofMillis(3)); // 시간 지연
  }
  ...
  - flatMap 연산자의 **출력**이 **다른 스레드의 구독자 핸들러**에 도착한다
  - 이벤트들 사이의 **발생 순서(the happens-before semantics)**를 보장 - Flux의 순서
...
- 처리량이 많은 시나리오의 경우 샘플링 기술을 적용해 일부 이벤트만 처리하는 것이 좋음
...
- 리액터 프로젝트 라이브러리는 리액티브 시퀀스를 **Blocking 구조로 변환**하기 위한 API를 제공함.
...
- ❗️https://stackoverflow.com/questions/56604767/what-is-the-different-between-using-the-dooneach-onerror-oncomplete-within-sub ➡ subscription을 통제할 수 없는 경우라거나, 위에서 말한대로. 파이프라인의 중간에 있는 원소를 처리할 때 필요함!
- hook!!

#### 데이터와 시그널 변환하기

- 때로는 데이터가 아니라 시그널을 이용해 스트림을 처리하는 것이 유용할 때가 있습니다

- 데이터 스트림을 시그널 스트림으로 변환, 역변환할 때 materialize 및 dematerialize 메서드를 제공합니다.
...
- 이처럼 매우 다양한 DSL을 제공하나, 작업에 적합한 연산자를 선택하는 데 고생하지 않을 정도로 라이브러리에 익숙해지려면 꽤 연습이 필요합니다.

### 코드를 통해 스트림 만들기

- 앞에서 리액티브 스트림을 만드는 방법을 배웠다. 그러나 때로는 stream 내에서 시그널을 생성하거나 객체의 라이프 사이클을 리액티브 스트림의 라이프 사이클에 바인딩하는? 보다 복잡한 방법이 필요함
- 리액터를 이용해 **stream을 프로그래밍 방식으로 생성하는 방법**을 알아보자

#### 팩토리 메서드 - push와 create



<img src="https://projectreactor.io/docs/core/release/api/reactor/core/publisher/doc-files/marbles/push.svg" alt="img" style="zoom:67%;" />

- **push** : Flux를 생성하고, 데이터를 수동으로 하나씩 추가하는 방식으로 스트림을 만든다.
  - **단일 스레드** 생성자를 적용해 Flux 인스턴스를 프로그래밍 방식으로 생성할 수 있음
  - 배압과 cancel에 대한 걱정 없이 비동기, 단일 스레드, 다중 값을 가지는 API를 적용하는 데 유용함
  - 구독자가 부하를 처리할 수 없는 경우 배압과 취소는 모두 큐를 이용해(버퍼링하여 처리) 처리된다
...
- push 팩토리 메서드는 기본 배압 및 취소 전략을 사용해 비동기 API를 적용할 때 유용하게 사용할 수 있음

<img src="https://projectreactor.io/docs/core/release/api/reactor/core/publisher/doc-files/marbles/createForFlux.svg" alt="img" style="zoom:67%;" />

- **create** : push와 비슷하게 동작하되, FluxSink 인스턴스를 추가로 직렬화하므로 **다른 스레드에서 이벤트를 보낼 수 있음.** 데이터 생성을 개발자가 제어하면서 비동기적인 데이터 스트림 생성

~~~java
//추가적인 핸들러를 등록해 리소스 정리를 활성화시킬수도 있음
Flux.create(emitter -> {
  emitter.onDispose(() -> log.info("Disposed")); // event를 emitter에게 push
})
  .subscribe(e -> log.info("onNext: {}", e));
~~~
...
- **generate** : 메서드를 호출하는 오브젝트의 내부 전달 상태를 기반으로 복잡한 시퀀스를 만들 수 있도록 설계
  ...
  - 이전 값을 기반으로 다음 내부 상태를 계산하고 onNext 신호를 다운스트림 구독자에게 전송하는 방식
  ...
  - 다음 새 값을 생성하기 전에 새 값이 동기적으로 구독자에게 전파
  - 각 출력 사이의 중간 상태를 필요로 하는 서로 다른 복잡한 리액티브 시퀀스 생성에 유용함
  - scan과의 차이점? 팩토리 메서드
...
- 리액티브 프로그래밍에서의 **try-with-resources 방식**(try 종료 시 자동으로 자원 해제해주는 기능) 접근법

  ~~~java
  // AutoClosable을 상속받아 구현하면 내부 리소스를 스스로 관리 할 수 있음
  public class Connection implements AutoCloseable {
      ...
      public Iterable<String> getData() {
        if(rnd.nextInt(10) < 3) throw new RuntimeException("Communication Error");
        return Arrays.asList("Some", "data");
      }
  }
  
  // 명령형 접근 방식에서의 Connection에서 데이터를 수신하는 코드
  // Connection이 AutoCloseable를 구현하고 있기에, close()를 안해주어도 된다.
  ...
  // 동일한 목표인 리액티브 코드
  Flux<String> ioRequestResults = Flux.using( // Connection 인스턴스 라이프 사이클을 스트림의 라이프 사이클에 래핑
  ...
  - 두 경우 모두 using 연산자를 이용해 새 Connection을 만든 다음, 워크플로를 실행하고 이전에 만든 Connection을 담음

#### usingWhen 팩토리를 사용해 리액티브 트랜잭션 래핑

- using 연산자처럼 usingWhen 연산자를 사용해 수동으로 자원을 관리할 수 있음

- using 연산자는(Callable 인스턴스를 호출해) 관리 자원을 동기적으로 검색하는 반면에 usingWhen 연산자는 (Publisher의 인스턴스에 가입해) **관리되는 리소스를 리액티브 타입으로 검색**

- 또한 usingWhen 연산자는 메인 스트림의 **성공 및 실패에 대해 각각 다른 핸들러**를 사용할 수 있음

- 즉, 완전한 **Non-Blocking Reactive Transaction**을 구현할 수 있음
  ...
  public class Transaction {
      ...
      public Flux<String> insertRows(Publisher<String> rows) {
          return Flux.from(rows)
              ...
              .flatMap(r -> {
                  if(random.nextInt(10) < 2) { // 랜덤으로 process를 실패시킴
      ...
      public Mono<Void> commit() {
          return Mono.defer(() -> {
              ...
              if(random.nextBoolean()) { // 랜덤으로 commit 실패
                  return Mono.empty();
              } else {
      ...
      public Mono<Void> rollback() {
  		return Mono.defer(() -> {
              ...
              if(random.nextBoolean()) { // 랜덤으로 rollback 실패
                  return Mono.empty();
              } else {
  ...
  // using에 비해 더 reactive한 방법을 자원의 라이프 사이클을 관리할 수 있음
  Flux.usingWhen( 
...
- 외부 서비스와 많은 커뮤니케이션을 하는 리액티브 애플리케이션을 설계할 때는 모든 종류의 예외 상황을 처리해야 함
- 다행히 onError 시그널은 리액티브 스트림 스펙의 필수 요소라서 예외를 처리할 수 있는 경로로 전파할 수 있음.
- 하지만 **최종 구독자가 onError 시그널에 대한 핸들러를 정의하지 않으면** onError는 UnsupportedOperationException을 발생시킴.
- 또한 리액티브 스트림은 onError가 스트림이 종료됐다고 정의하기 때문에 시그널을 받으면 **시퀀스가 실행을 중지**함
- 이에, onError 시그널을 받으면 다른 방식으로 대응할 수 있음
  ...
  - **retry** 연산자 : 오류 시그널을 보내는 경우 소스 리액티브 시퀀스를 다시 구독
  - **retryBackOff** 연산자 : 지수적인 백오프 알고리즘을 지원해 재시도할 때마다 대기 시간을 증가
- 또한 비어 있는 스트림이 때때로 원하는 입력이 아닐 수 있기에
  - defaultIfEmpty 연산자 : empty로 들어올 시 기본값 반환
  - switchIfEmpty 연산자 : empty로 들어올 시 완전히 다른 리액티브 스트림으로 반환
  - timeout 연산자 : 작업 대기 시간을 제한하고 TimeoutException을 발생시킬 수 있음


~~~java
public Flux<String> recommendedBooks(String userId) {
    return Flux.defer(() -> {
        if(random.nextInt(10) > 7) { // 적당히 높은 확률로 추천 실패
            return Flux.<String>error(new RuntimeException("Err"))
                	.dellaySequence(Duration.ofMillis(100));
        } else {
            return Flux.just("The little prince", "Scala").delayElements(Duration.ofMillis(50));
        }
...
Flux.just("user-1") 
    .flatMap(user ->
      recommendedBooks(user)
        ...
        .timeout(Duration.ofSeconds(3))// 3초동안 해결되지 않을 시 오류 시그널 발생
...
- 이렇듯, 리액터 프로젝트는 **예외 처리를 돕고** 응용프로그램의 **복원력**을 향상시키는데 도움이 되는 다양한 도구를 제공

#### Back Pressure 다루기

- Reactive stream 스펙에서 프로듀서와 컨슈머 간의 의사소통하는데 있어 back pressure이 있더라도, **컨슈머에서 오버플로가 발생할 가능성은 여전히 존재**함.
- 일부 컨슈머는 제한되지 않은 데이터에 순진하게 응답한 다음 생성된 부하를 처리하지 못함
- 이에, 세심한 back pressure 처리를 위한 연산자가 있다.
  ...
  - onBackPressureError : 다운스트림 컨슈머가 처리를 계속 유지할 수 없으면 **Publisher는 오류를 발생**
- 또한 **속도를 제한**하여 back pressure를 관리할 수도 있다.
  - limitRate(n) : 다운스트림 수요를 **n보다 크지 않은 작은 규모로 나눔.**
  - limitRequest(n) : 다운스트림 **컨슈머의 수요를 제한**할 수 있음
    - limitRequest(100) => 100개의 이벤트를 전송한 후 Publisher는 스트림을 성공적으로 닫음
...
- cold publisher : **구독자가 나타날 때마다 데이터를 처음부터 다시 생성하는 스트림**. 구독자 없이는 데이터가 생성되지 않음
  ...
  onNext: 63c8d67e-86e2-48fc-80a8-a9c039b390c
  Generating new items
  onNext: 52232746-9619-4b5e-b6b9-b0a2fa76079a
...
- 각 구독자를 위해 중복된 데이터를 생성하지는 않으려고 하며, 이러한 용도로 **ConnectableFlux**가 있음.

- ConnectableFlux를 이용하면 가장 수요가 많은 데이터를 생성하고 다른 **모든 가입자가 데이터를 처리할 수 있도록 캐싱**

  <img src="https://projectreactor.io/docs/core/release/api/reactor/core/publisher/doc-files/marbles/publish.svg" alt="img" style="zoom:50%;" />
...
- ConnectableFlux를 사용하면 다양한 데이터 캐싱 전략을 쉽게 구현할 수 있으나 리액터에는 **cache 연산자**를 통해 이벤트 캐싱이 가능하다.

- 캐시가 보유할 수 있는 데이터 양과 캐시된 각 항목의 만료 시간을 조정할 수 있음
  ...
  [S 3] onNext: 0
  [S 3] onNext: 1
  // 처음 두 구독자가 첫 번째 구독의 동일한 캐시된 데이터를 공유했음
  ~~~
...
- **share 연산자**를 이용하면 cold publisher를 hot publisher로 변환할 수 있다.

  - 구독자가 각 신규 구독자에게 이벤트를 전파하는 방식으로 작동
  - publish랑 같은거 아닌가? 싶은데 publish의 경우 connect()를 호출해야해서 share가 좀 더 편리하다고 함..(chatgpt)
...
- 복잡한 리액티브 워크플로를 구축할 때 종종 서로 다른 위치에서 **동일한 순서의 연산자를 사용해야 할 때**가 있다.

- transform 연산자와 compose 연산자를 이용해 데이터 스트림을 변환하여 **별도의 객체로 추출**할 수 있다. -> **재사용성**!!
  ...
  Function<Flux<String>, Flux<String>> logUserInfo = 
    stream -> stream.index() //열거형 전환, return 값은 Flux<Tuple2<Long, String>>
...
- Processor는 Publisher이며 Subscriber이고 Processor 인스턴스는 구독이 가능하고, 시그널을 수동으로 보낼 수 있음
- 리액터 개발팀은 **Processor를 사용 어려움 및 오류 가능성의 이유로 사용하지 않을 것을 권장**하고 있음
- 리액터의 프로세서들은 생략...
...
- 이제 **리액티브 스트림**의 수명 주기, 멀티스레딩, 내부 최적화 방식에 대해 알아보자.

#### 리액티브 스트림의 수명 주기

- 리액터에서 리액티브 타입의 수명 주기를 이해해보자

##### 조립 단계

- 스트림 수명 주기의 첫 번째 부분은 조립 단계

- 학습했듯이, 리액터는 복잡한 처리 흐름을 구현할 수 있는 연쇄형 API를 제공함.

- 빌더 API처럼 보이지만, 차이점은 리액터 API는 **불변성(Immutability)을 제공**함. -> 각각의 연산자가 새로운 객체를 생성
...
- stream 수명 주기에서 조립 단계가 중요한 이유는 stream의 타입을 확인해 연산자를 서로 바꿀 수 있기 때문?
  ...
  - 매개변수로 받은 Flux가 FluxConcatArray 인스턴스인 경우에는 FluxConcatArray(FluxConcatArray(FluxA, FluxB), FluxC)로 동작하는 것이 아니라 하나의 FluxConcatArray(FluxA, FluxB, FluxC)로 만들어 전체적인 성능을 향상시킨다
...
- 실행 플로를 만들기 위해 내부적으로 **Publisher를 다른 Publisher에게 전달**하는데, 일련의 **Publisher 체인**이 있다고 할 수 있다.
...
- Publisher와 Subscriber 간에 실제 신호가 교환됨.(onSubscribe 시그널과 request 시그널)
  ...
  MapSubscriber(FilterSubscriber(Subscriber))
    .onSubscribe(new ArraySubscription()) {
  ...
  - 구독이 모든 구독자 체인을 통과하고, 체인에 포함된 각 구독자가 지정된 구독을 자신의 표현으로 래핑

- 메시지 수신을 위해 Subscription#request 메서드가 호출해 전송을 시작해야 함

- 모든 구독자를 통해 데이터를 보내는 프로세스를 설명하는 의사코드
  ...
  ArraySubscription.request(10) {
    MapSubscriber(FilterSubscriber(Subscriber)).onNext(1) {
      FilterSubscriber(Subscriber).onNext("1") {
        //필터 처리
        // 원소가 일치하지 않으면 추가 데이터 요청
        MapSusbscription(AraySubscription(...)).request(1) {...}
      }
    }
  }
  
  ArraySubscription.request(20) {
  	// 데이터 변환 로직
    FilterSubscriber(Subscriber).onNext("20") {
      //필터 처리
      // 원소가 일치하면 다운스트림 구독자에게 전송
  ...
  - 런타임 중에 데이터는 소스로부터 각 Subscriber 체인을 거쳐 단계마다 다른 실행을 함

- 런타임 단계가 중요한 이유는 런타임 중에 signal 교환량을 줄이기 위한 최적화를 적용할 수 있기 때문

  - Subscription#request 호출 횟수를 줄임으로써 스트림의 성능을 향상시킬 수 있음
  - Subscription#request 메서드 호출 시 요청 수요를 저장하는 volatile 필드에 쓰기가 발생해서 비용이 많이 드는 연산임

  <img src="./image/그림 4.8 리액티브 플로의 생명 주기.png" alt="그림 4.8 리액티브 플로의 생명 주기" style="zoom:50%;" />
...
- publishOn 연산자는 **런타임 실행의 일부를 지정된 워커로 이동**할 수 있게 해준다.

<img src="https://projectreactor.io/docs/core/release/api/reactor/core/publisher/doc-files/marbles/publishOnForFlux.svg" alt="img" style="zoom:50%;" />
...
- Scheduler : 리액터 프로젝트에서 worker 또는 worker pool을 나타내는 인터페이스. 스트림에 대한 특정 워커를 선택하는 용도로 사용됨.
  ...
  <img src="./image/그림 4.9 리액터 publishOn 연산자의 내부 구조.png" alt="그림 4.9 리액터 publishOn 연산자의 내부 구조" style="zoom:30%;" />

  

  - publishOn 연산자는 런타임 실행에 초첨을 맞춘다
  - publishOn 연산자는 내부적으로 **전용 워커가 메시지를 하나씩 처리할 수 있도록 새로운 원소를 제공하는 큐**를 가지고 있음.
  - 리액티브 스트림의 모든 원소는 하나씩 처리되므로 항상 모든 이벤트에 순서를 엄격하게 정의할 수 있음 -> **직렬성(serializability)**
  - 다시 말해, 원소가 publishOn에 오면 큐에 추가되고 차례가 되면 큐에서 꺼내서 처리한다

  ##### publishOn 연산자를 이용한 병렬 처리

  - 리액터 프로젝트는 publishOn 연산자를 사용함으로써 미세한 규모의 조정 및 처리 흐름의 **병렬 처리를 가능하게** 해서 리액티브 프로그래밍 패러다임에 부합하고 있음
  - 아래 두 그림을 비교해보자.(순수한 동기 처리, 비동기 처리)
  ...
  <img src="./image/그림 4.11 publishOn 연산자에 의한 스트림 처리의 변화.png" alt="그림 4.11 publishOn 연산자에 의한 스트림 처리의 변화" style="zoom: 33%;" />

- 동기 처리

  - 스트림에서 원소의 동기 처리 특성으로 인해 각 변환 단계에 원소를 하나씩 이동
  - 다음 처리를 시작하려면 이전 작업을 완료해야 함

- publishOn 연산자를 포함시킨 동일한 처리

  - 원소의 처리 시간을 동일하게 유지하되, **처리 단계 사이에 비동기 영역**(publishOn 연산자로 표시)을 추가해 병렬 처리를 할 수 있음
  - 왼쪽 프로세스가 오른쪽 프로세스를 기다릴 필요가 없어지고, **독립적으로 작업하여 병렬 처리**가 이루어질 수 있음!

##### subscribeOn 연산자

<img src="https://projectreactor.io/docs/core/release/api/reactor/core/publisher/doc-files/marbles/subscribeOnForFlux.svg" alt="img" style="zoom:50%;" />

- publishOn과 달리 subscriberOn을 사용하면 **구독 체인에서 워커의 작업 위치를 변경할 수 있음**

- subscribeOn은 함수를 실행해 스트림 소스를 만들 때 유용하게 사용할 수 있음
  ...
  // 내부적으로 Mono.fromCallable이 하는 작업
  public void subscribe(Subscriber actual) { 
    ...
  	Subscription subscription = ...;
    try {
  		T t = callable.call();  // subscribe 메서드에서 callable이 실행됨 -> publishOn을 사용해 Callable이 실행될 워커를 변경할 수 있음
...
- subscribeOn과 publishOn의 실행 모델 비교

  <img src="./image/그림 4.12 publishOn 연산자의 내부.png" alt="그림 4.12 publishOn 연산자의 내부" style="zoom:30%;" />
...
- parallel 연산자는 하위 스트림에 대한 플로 분할과 분할된 플로 간 균형 조정 역할을 한다. -> 작업 분배
...
- 이용 시, ParallelFlux를 동작시킨다.
  ...
  Flux.range(0, 10000)
    	...
    	.runOn(Schdulers.parallel()) //publishOn을 내부 Flux에 적용
    	.map()
    	.filter()
    	.subscribe();
  ~~~
...
- 같은 일을 할 수 있는 ThreadLocal 대신에 왜 Context가 또 필요한지 궁금할 수도 있습니다.
  ...
  - 대다수의 프레임워크는 ThreadLocal을 사용해 사용자 요청에 SecurityContext를 전달하고, 각 처리 지점에서 사용자의 액세스 권한이 적절한지 확인합니다.
    ...
    - 인증 이후 스트림 처리는 일반적으로 단일 스레드에서 이루어짐
    - Threadlocal 사용했을 때의 장점? 스레드 간 독립성 유지, 스레드 내 직접 접근으로 인한 성능 향상 등등

<img src="https://oopy.lazyrockets.com/api/v2/notion/image?src=https%3A%2F%2Fs3-us-west-2.amazonaws.com%2Fsecure.notion-static.com%2Fabf1b3ea-a402-48bb-879c-bf099a25fabb%2F1.png&blockId=e5a25e72-1e6e-4cac-9901-bf25d6b5d13f" alt="img" style="zoom:50%;" />

- 허나 이는 단일 스레드 환경에서 이용할 때만? 제대로 동작하는 방식
...
- 이를 리액터 Context는 아래 코드와 같이 해결!
  ...
  - Mono.subscriberContext 연산자를 통해 Context에 access 할 수 있음
  ...
  - doOnNext에서 데이터 전송을 위해 왜 또 map을 사용할까?
    - Context는 멀티스레드 액세스 모델임을 고려해서 만들었기에 **Immutable 객체**여서 새로운 요소를 추가하면 Context는 새로운 인스턴스로 변경됨
    - Context는 조립 단계, 구독 단계를 포함해 전체 런타임동안 사용할 수 있는 데이터를 동적으로 제공하는 **유일한** 방법
    - 각 Subscriber가 별도의 Context를 가지게 해줄 수 없음(많은 접근을 원하지 않는?)
    - 각 Subscriber에게 별도의 컨텍스트가 제공될 수 있는 유일한 단계는 구독 단계이다.
  - 구독 단계 동안 Subscriber는 Publisher 체인을 따라 스트림 아래 쪽에서부터 위쪽으로 이동하면서 추가 런타임 로직을 적용하는 로컬 Subscriber로 각 단계를 래핑함.

- Subscriber가 스트림을 통해 추가 Context 객체를 전달하기 위해 리액터는 **CoreSubscriber**라는 특정 Subscriber 인터페이스 구현체를 사용한다.
...
- 현재 Context를 수정할 수 있는 유일한 연산자는 subscriberContext이며, 이 연산자는 다운스트림의 Context와 전달된 매개변수를 병합하는 방식으로 CoreSubscriber를 구현
...
- 이는 액세스 가능한 Context 객체가 스트림의 다른 지점에서는 동일한 객체가 아닐 수 있다는 것을 의미
  ...
  - top에는 Context에 이 스트림에서 사용할 수 있는 전체 컨텍스트가 포함되어 있음
  - middle에는 스트림에서 정의한 컨텍스트와 컨텍스트 컨슈머만 액세스할 수 있으며, 가장 아래(initial)는 Context가 비어있음.

- 일반적으로 Context는 리액티브 시스템을 구축하기 위한 리액터 프로젝트를 사용하게 유인하는 킬러 기능

- 사용자 요청 처리 중에 특정한 상황을 처리하기 위한 기능을 구현하는 것처럼 **각 요청에서 데이터에 엑세스해야 하는 경우에 유용**함

- 특히 Spring Security에서 많이 사용!

### 프로젝트 리액터의 내부 구조

- 리액터 프로젝트 3버전의 핵심 개선 사항은 **리액티브 스트림 수명 주기** 및 **연산자 융합**이다.

#### 매크로 퓨전

- 주로 조립 단계에서 발생하며 **연산자를 다른 연산자로 교체하는 것**

  - Flux 내부의 일부 연산자(just, empty, error)들은 다른 변환 작업과 함께 사용되는데, 이에 **오버헤드**를 줄이는 것이 중요하다
...
- 일반적으로 리액터 프로젝트에 적용된 매크로 퓨전의 목적은 **조립된 흐름을 최적화**하는 것이며, 이를 위해 별도의 강력한 도구슬 사용하는 것이 아니라 간단하고 비용 효율적인 솔루션을 사용할 수 있다

#### 마이크로 퓨전

- 매크로 퓨전에 비해 좀 더 복잡한 최적화이며 **런타임 최적화 및 공유 리소스 재사용**과 관련 있음

  <img src="/Users/seongjayong/Learning/book-spring-reactive/image/그림 4.13 트럭 예제와 조건부 문제.png" alt="그림 4.13 트럭 예제와 조건부 문제" style="zoom:30%;" />

- 첫번째 예제. 공장에서 만든 상품을 상점에 팔기 위해서는 검사 부서를 거쳐야 한다.

- 검사 부서를 매번 거쳐야하는 단점 때문에 공장에서 직접 검사관을 고용하였고, 이에 공장에서 모든걸 확인 후 상점으로 보냄.
...
- 예제의 문제상황? 
  ...
  - 다운스트림 가입자는 소스에 특정 수의 원소 요청 ➡️ 원소가 조건부 연산자를 통해 이동 ➡️ 거부된 항목에 대해 필터 연산자가 request() 메서드를 통해 부족한 항목 보충 ➡️ request() 메서드 실행은 추가적인 CPU 부하를 발생시킴
  - 이러한 이유로 마이크로 퓨전 유형의 **ConditionalSubscriber**가 존재함

- ConditionalSubscriber로 최적화하여 소스 측에서 조건을 바로 확인하고 추가적인 request 메서드 호출 없이 필요한 개수를 전송

- 두번째 예제(좀 더 어려움) : 연산자 간 비동기 경계와 관련 -> 몇 개의 비동기 경계가 있는 연산자 체인을 가정해 보자.
  ...
  // 1. concatMap 연산자. 잠재적으로? 업스트림에 들어오는 각 원소마다 n개의 원소를 생성할 수 있음
