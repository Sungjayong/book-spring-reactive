...
- 때로는 stream 내에서 시그널을 생성하거나 객체의 라이프 사이클을 리액티브 스트림의 라이프 사이클에 바인딩하는? 보다 복잡한 방법이 필요함
- 리액터를 이용해 (원래 있던 stream을 조작하는게 아니라) **stream을 프로그래밍 방식으로 생성하는 방법**을 알아보자
...
- push 팩토리 메서드는 배압과 cancel에 대한 걱정 없이 비동기, 단일 스레드, 다중 값을 가지는 API를 적용하는 데 유용
