...
- 런타임 단계가 중요한 이유는 런타임 중에 signal 교환량을 줄이기 위한 최적화를 적용할 수 있기 때문
  ...
  <img src="/Users/seongjayong/Learning/book-spring-reactive/image/그림 4.8 리액티브 플로의 생명 주기.png" alt="그림 4.8 리액티브 플로의 생명 주기" style="zoom:50%;" />
...
- subscribeOn과 publishOn의 실행 모델 비교
  ...
  <img src="/Users/seongjayong/Learning/book-spring-reactive/image/그림 4.12 publishOn 연산자의 내부.png" alt="그림 4.12 publishOn 연산자의 내부" style="zoom:30%;" />
...
- 이를 리액터 Context는 아래 코드와 같이 해결!
  ...
  - doOnNext에서 데이터 전송을 위해 왜 또 map을 사용할까?
    - Context는 멀티스레드 액세스 모델임을 고려해서 만들었기에 Immutable 객체여서 새로운 요소를 추가하면 Context는 새로운 인스턴스로 변경됨
    - Context는 조립 단계, 구독 단계를 포함해 전체 런타임동안 사용할 수 있는 데이터를 동적으로 제공하는 **유일한** 방법
    - 각 Subscriber가 별도의 Context를 가져야 하는 경우에는 유용하지 않을 수도 있음(많은 접근을 원하지 않는?)
