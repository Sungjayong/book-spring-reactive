...
- 시스템의 기능 요소를 격리해 모든 내부 장애를 격리하고 독립성을 확보함으로써 달성 가능 => 마이크로서비스
  - 댓글 서비스에 장애가 발생하더라도 다른 서비스에는 아무런 영향없이 동작
  
- **탄력성**과 **복원력**이 밀접하게 결합되어 있으며, 이 두 가지 모두 충족되어야 시스템의 진정한 응답성을 달성할 수 있다.

### 메시지 기반 통신

- 분산 시스템에서 컴포넌트를 연결하는 방법, 낮은 결합도, 시스템 격리 및 확장성 유지를 동시에 달성할 지
...
/<img src="https://blog.kakaocdn.net/dn/b98rl5/btroVrU8BYn/RJYgbIGAMqXY6gVa1k5R3k/img.jpg" alt="동기와 비동기, 블로킹과 논블로킹 개념 정리" style="zoom:50%;" />

- 리소스 활용도가 높다 => 응답성이 높다
  
- 일반적으로 분산 시스템에서 서비스 간 통신을 할 때 자원을 효율적으로 사용하기 위해서 메시지 기반(message-driven) 통신 원칙을 따른다.
  - 구성요소들은 메시지가 도착하면 이에 반응

- 메시지 기반 통신을 수행하는 방법의 하나로 **메시지 브로커(message brocker)**가 있다.
  - 대기열 모니터링으로 시스템이 부하 관리 및 탄력성 제어 가능
  - 명확한 흐름 제어 제공

최종적인 리액티브 시스템의 기본 원리

<img src="https://blog.kakaocdn.net/dn/bg4cS9/btq3TVPZ2In/dVmz0NxWtGo9urcLVUB7W0/img.png" alt="img" style="zoom:80%;" />
...
- 구현 관점에서 Reactive 접근법이 적용이 어떻게 되는지 알아보자.
...
- 스프링 프레임워크만으로는 견고한 리액티브 시스템을 구축하는 데는 몇 가지 제약이 있음
- Reactive System 구축을 위해 Spring Cloud가 생겨났음.
...
- 즉, 구성 요소 수준에서도 Reactive 설계 및 구현을 제공하는 것이 중요하다!
...
- OrderService 로직을 처리하는 동안 스레드가 차단됨.
...
class AyncShoppingCardService implements ShoppingCardService {
  public void calculate(Input value, Consumer<Output> c) {
    new Thread(() -> {
      ...
      ...
      c.accept(result); //callback 함수 호출
    }).start();
  }
}
~~~

- callback 함수를 활용한 비동기식 구현
...
- Future 클래스 사용으로 결괏값 반환을 지연시킬 수 있음
...
- 그 외에 ListenableFuture 등을 사용할 수 있지만.. 생략



- 멀티 스레딩 할 때는 여러 스레드에서 공유 메모리에 액세스, 동기화, 오류 처리 등과 같은 많은 작업을 생각해야 함.
- 제한된 크기 스레드 풀을 제공하는 기존 모델로 전환하고 요청에 대해 미리 구성된 대기열을 사용하면 클라이언트가 응답을 받기까지의 대기 시간이 길어지고 평균 응답 시간이 길어지면서 결국 응용 프로그램이 응답하지 않을 수 있음.
- 이를 위해 Reactive 선언문은 Non-blocking 작업을 사용하도록 권장
- 비동기 처리는 일반적인 요청-응답 패턴 뿐만 아니라 데이터의 연속적인 스트림에서도 처리할 수 있다. -> 이런 사례를 Reactive Programming에서 **변형 단계**를 포함함.
- 결과적으로, Reactive Programming은 Reactive System의 설계 요구 사항에 적합한 좋은 기술임.
...
- 아키텍처 패턴으로서의 리액티브 시스템이 있고, 프로그래밍 기술로서의 리액티브 프로그래밍이 있으며 다르고, 시스템적으로 프로그래밍적으로도 Reactive하게? 처리할 수 있게 하는 것이 중요하다.
