...
- DB 종류 중 임베디드 데이터베이스가 있으며, 응용 프로그램 프로세스 내에서 실행되므로 네트워크를 통한 통신이 필요하지 않다 ➡️ H2 DB
...
~~~java
@Document(collection = "book") //@Entity 대신 @Document 사용
public class Book {
  @Id
  private ObjectId id;
  
  @Indexed // 인덱스를 사용하기 위해 붙임
  private String title;
...

{
  ...
  "title" : "The Expanse: Leviathan Wakes", "pubYear" : 2011,
	...
	"_class" : "org.rpis.chapters.chapter_07.mongo_repo.Book" // Object-Document Mapping 되는 자바 클래스를 설명하는 _class 필드를 추가한다.
}
~~~
...
- 현재 스프링 데이어에는 몇 가지 리액티브 데이터 커넥터가 있음..!
...
- 이 방법은 TCP 소켓의 수를 줄이고 그림과 같이 클라이언트와 서버 모두에 대해 더 큰 동시성을 허용함
  ...
  <img src="/Users/seongjayong/Learning/book-spring-reactive/image/그림 7.15 일반적인 데이터베이스 통신과 멀티플렉싱을 지원하는 통신 프로토콜 .png" alt="그림 7.15 일반적인 데이터베이스 통신과 멀티플렉싱을 지원하는 통신 프로토콜 " style="zoom:40%;" />
...
- 제목 및 새로운 발행 연도를 모두 받은 후에만 데이터베이스에 책을 검사하라는 명령을 보냄
